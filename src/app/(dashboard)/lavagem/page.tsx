        ...
        <div className="grid lg:grid-cols-2 gap-6">
          <div>
            ...
            <div className="space-y-3">
              {waiting.map((order) => (
                <OrderCard key={order.id} order={order} onStatusChange={updateStatus} onFinishAndSend={finishAndSend} sending={sending === order.id} />
              ))}
...
function OrderCard({ order, onStatusChange, onFinishAndSend, sending }: any) {
          ...
          {order.status === "IN_PROGRESS" && (
            <>
              <Button
                size="sm"
                variant="success"
                onClick={() => onFinishAndSend(order.id)}
                disabled={sending}
                className="gap-1"
              >
